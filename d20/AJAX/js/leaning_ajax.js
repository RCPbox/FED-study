
(function(global, undefined){
	'use strict';

	// Ajax를 수행하기 위한 XMLHttpRequest 객체 생성 (생성자 함수로부터)
	// 보통 실행연산자 안붙이는데 넘기는 인자가 있을때 사용.
	var xhr = new XMLHttpRequest,
		jsonXhr = new XMLHttpRequest;

	// xhr 객체 콘솔에 로그(기록)
	// console.log(xhr);

	// console.log(xhr.readyState);//0

	// console.log('통신 준비');

	// xhr.open()
	// 첫번째 전달인자: 어떤 방식(Method)으로 서버와 통신할 것인가? GET/POST
	// 두번째 전달인자: 어떤 데이터(Data)를 가져올 것인가?
	// 세번째 전달인자: 비동기적으로 통신할 것인가?
	xhr.open('GET', 'data/data.txt'); // 디폴트값 true: 비동기 ,false: 동기 통신 방식
	jsonXhr.open('GET', 'data/data.json');// 모델데이터 ajax로 호출
	// console.log('통신을 위한 오픈');

	// 서버 통신
	// xhr.send(); // 보낼때.
	jsonXhr.send();

	jsonXhr.onreadystatechange = function() {
		// 완벽하게 통신하기 위한 조건.
		// 0 (uninitialized) , 1 (loading) , 2 (loaded) ,3 (interactive) , 4 (complete)
		if(this.status === 200 && this.readyState === 4) {
			var recieveData = this.responseText;
			var convertObject = JSON.parse(recieveData);// 문자를 객체화.
			console.log(convertObject);// 객체 출력
		}
	};

	// 상태 변경을 감지하여 이벤트 핸들러 실행. 이벤트 감지.
	xhr.onreadystatechange = function() {
		if(this.status === 200 && this.readyState === 4) {
			// console.log(this.responseText);
			var html = ['<div id="ajax-data">', '<p>'+ this.responseText +'</p>','</div>'].join('');
			document.body.innerHTML = html + document.body.innerHTML;
		}
	};

	// 한번만 수행. 다 로딩이 끝나고.
	// xhr.onload = function() {
	// 	// 서버가 보낸 수신된 데이터 파일 내용 출력
	// 	var resTxt = xhr.responseText;

	// 	console.log(resTxt);
	// }

	// console.log('통신');

})(window);