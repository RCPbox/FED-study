...
function BioCreature(init_settings) {
	...
	// -----------------------------------------------------
	// 감춰진 멤버에 접근/조작할 수 있는 특별한 멤버(Privileged Member)// 단점 메모리 많이 사용.
	...
	// ( obj객체 , property문자 ,descriptor객체 )
	Object.defineProperty(this, 'birth_place', {// this 동적으로 생성된 객체
		'value': _birth__place,
		'configurable' : true,// 삭제 가능한지.
		// 'enumerable'   : true,// 열거 가능.
		// 'writable'     : true // 쓰고 읽기 가능. default:false.
	});

	// 객체의 속성이 열거(Enumerable) 가능하니 체크하려면...
	// 해당 객체.propertyIsEnumerable(값); // true or false

	// 'value'와 'get'을 함께 사용할 수는 없다.
	Object.defineProperty(this, 'leg_count', { // 함수는 아니다. 값을 가져오기만 할뿐.
		'get': function() {
			return _leg__count;
		},
		'set': function(value) {
